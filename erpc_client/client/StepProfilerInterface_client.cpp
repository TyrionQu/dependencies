...
void StylusProfiler_client::SingleAcquisition(void)
{
    erpc_status_t err = kErpcStatus_Success;
...
// StylusProfiler interface SetMeasurementPara function client shim.
int8_t StylusProfiler_client::SetMeasurementPara(float _sample, float * _speed, float _length, float _duration, float * _resolution)
{
    erpc_status_t err = kErpcStatus_Success;
    ...
    codec->startWriteMessage(message_type_t::kInvocationMessage, m_serviceId, m_SetMeasurementParaId, request.getSequence());
    ...
    // Invoke error handler callback function
    m_clientManager->callErrorHandler(err, m_SetMeasurementParaId);
